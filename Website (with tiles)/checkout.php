...
function performConcurrencyCheck($conn, $IN_ORDER_ID, $IN_ORDER_QUANTITY, $FOOD_ID)
{
...
if (isset($_SESSION['prsn_id'])) {
    $sql = "SELECT io.IN_ORDER_ID, f.FOOD_NAME, f.FOOD_ID, f.FOOD_IMG, f.FOOD_PRICE, f.FOOD_STOCK, f.HOURLY_CAP, io.PRSN_ID, io.IN_ORDER_QUANTITY, io.IN_ORDER_TOTAL 
FROM in_order io
        ...
        if (mysqli_num_rows($res_orders) > 0) {
            while ($order_row = mysqli_fetch_assoc($res_orders)) {
                ...
                if ($IN_ORDER_QUANTITY > $avail) {
                    ...
                    // <script>
                        // alert("Ooops! The maximum capacity has been reached already. Someone might have ordered before you. Please select another date.")

                    // Swal.fire({
                    //     icon: "error",
                    //     title: "Oops!",
                    //     text: "The maximum capacity has been reached already. Someone might have ordered before you. Please select another date.",
                    //     iconColor: 'yellow',
...
if (isset($_POST['submit'])) {
    ...
    foreach ($flag as $x) {
        if ($x == false) {
            ...
            header("location:cart.php");// Disallow checkout if any item has insufficient stock
            break; // Exit the loop
        }
    }
                                    ...
                                    <div class="table-wrap">
                                        <table class="order">
                                            ...
                                            <tbody>
                                                <?php
                                                if (isset($_SESSION['prsn_id'])) {
                                                    $sql = "SELECT io.IN_ORDER_ID, f.FOOD_NAME, f.FOOD_ID, f.FOOD_IMG, f.FOOD_PRICE, f.FOOD_STOCK, io.PRSN_ID, io.IN_ORDER_QUANTITY, io.IN_ORDER_TOTAL 
            FROM in_order io
            LEFT JOIN placed_order po ON io.placed_order_id = po.placed_order_id
            JOIN food f ON io.FOOD_ID = f.FOOD_ID
            WHERE io.IN_ORDER_STATUS != 'Delivered' 
